 ...
 * are moved to different columns, emailCol and balanceCol must be updated!
 */

const google = require('@googleapis/sheets')

// Column of the sheet containing emails
const emailCol = 0;
// Column of the sheet containing names
const nameCol = 1;
// Column of the sheet containing balances
const balanceCol = 11;
// Column containing starting balance
const startingBalanceCol = 2;
 ...
 * 
 * @param {string} email An email (case sensitive) in the balance sheet
...
const getAllData = async (email) => {
    ...
    for (let i = 0; i < response.data.values.length; ++i) {
        if (response.data.values[i][emailCol].normalize() === email) {
            balance = parseInt(response.data.values[i][balanceCol]);
            startingBalance = parseInt(response.data.values[i][startingBalanceCol]);
            name = response.data.values[i][nameCol];
            return {name, balance, startingBalance};
        }
    }
    return null;
};
